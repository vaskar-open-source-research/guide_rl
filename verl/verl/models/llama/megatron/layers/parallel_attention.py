...
# -----------------------------------------------------------------------------
# Helper functions for RoPE scaling (adapted from the online references)
# -----------------------------------------------------------------------------

def default_rope_init(dim: int, base: float = 10000.0, device: Optional[torch.device] = None) -> torch.Tensor:
    """
    Computes the default inverse frequency for Rotary Position Embeddings.
    """
    return 1.0 / (base ** (torch.arange(0, dim, 2, dtype=torch.int64, device=device).float() / dim))


def llama3_rope_init(
    ...
    """
    Adjusts the default inverse frequency in order to implement Llama 3 rope scaling.
    
    Args:
        inv_freq: The default inverse frequency vector.
        factor: Scaling factor to use when wavelengths are high.
        high_freq_factor: Threshold defining high frequencies (wavelengths below this value remain unchanged).
        low_freq_factor: Threshold defining low frequencies (wavelengths above this value will be scaled).
        original_max_position_embeddings: The max position embeddings from the original model.
    
    Returns:
        Modified inverse frequency vector.
    """
    # Compute the wavelength thresholds.
    low_freq_wavelen = original_max_position_embeddings / low_freq_factor
    high_freq_wavelen = original_max_position_embeddings / high_freq_factor

    # Compute the wavelengths associated to each element in inv_freq.
    wavelen = 2 * math.pi / inv_freq
    # For positions with wavelen greater than the low frequency threshold, scale by factor.
    out_inv_freq = torch.where(wavelen > low_freq_wavelen, inv_freq / factor, inv_freq)
    ...
    smoothed_inv_freq = ((1 - smooth_factor) * (out_inv_freq / factor)) + (smooth_factor * out_inv_freq)
    # Identify positions in the medium-frequency range.
    is_medium_freq = ~(wavelen < high_freq_wavelen) * ~(wavelen > low_freq_wavelen)
    out_inv_freq = torch.where(is_medium_freq, smoothed_inv_freq, out_inv_freq)
    return out_inv_freq


# -----------------------------------------------------------------------------
# Modified Llama3ScalingRotaryEmbedding using Llama 3 rope scaling
# -----------------------------------------------------------------------------

class Llama3ScalingRotaryEmbedding(LlamaRotaryEmbedding):
    """
    LlamaRotaryEmbedding extended with Llama 3 rope scaling.

    This implementation uses the helper functions above (which follow the logic
    used in xformers, HuggingFace Transformers, and GPT-NeoX) to adjust the
    base inverse frequency.
    
    Additional parameters:
      - scaling_factor: factor used for scaling high frequencies.
      - high_freq_factor: threshold factor for high frequencies (wavelengths below this remain unchanged).
      - low_freq_factor: threshold factor for low frequencies (wavelengths above this are scaled).
    """
    def __init__(
        self,
        dim,
        ...
        low_freq_factor: float = 8.0,
    ):
        # Store additional parameters for the llama3-specific scaling
        self.scaling_factor = scaling_factor
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        # Compute the default inverse frequency vector.
        inv_freq = default_rope_init(self.dim, base=self.base, device=device)
        # Apply the Llama3 scaling adjustments.
        inv_freq = llama3_rope_init(
            inv_freq=inv_freq,
        ...
        # Precompute the cosine and sine embeddings.
