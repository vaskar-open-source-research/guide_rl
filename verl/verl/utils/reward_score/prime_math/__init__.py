...
def match_answer(response, response_format) -> Tuple[bool, str]:
    ...
    elif response_format == "reasoning_format":
        
        answer_idx = response.rfind("A:")
        is_matched = answer_idx != -1
        if answer_idx != -1:
            response = response[answer_idx + 2:].strip()
        
        return is_matched, response


def compute_score(model_output: str, ground_truth: str, response_format: str = "default") -> Tuple[bool, bool, str]:
    model_output = str(model_output)
    ground_truth = str(ground_truth)

    format_correctness, extracted_model_output = match_answer(model_output, response_format)
