...
def find_latest_ckpt_path(path, directory_format="global_step_{}", resume_from_iteration=None):
    if path is None:
        return None
    
    if resume_from_iteration is not None:
        ckpt_path = os.path.join(path, directory_format.format(resume_from_iteration))
        return ckpt_path
