...
def compute_rloo_returns(data, eos_mask: torch.Tensor, n_samples: int, config: dict):
    ...
    with torch.no_grad():
        ...
        for k, v in data.batch.items():
            ...
            for start_pos in range(0, reward_tensor.shape[0], n_samples):
                cur_rewards_mean = torch.cat([ reward_tensor[pos:pos + 1][ reward_mask[pos:pos + 1] ].mean(dim=0, keepdim=True) for pos in range(start_pos, start_pos + n_samples) ], dim=0)
                cur_rewards_sum = cur_rewards_mean.sum()
                cur_reward_baseline = cur_rewards_sum / (n_samples - 1)
                reward_tensor[start_pos:start_pos + n_samples][reward_mask[start_pos:start_pos + n_samples]] = reward_tensor[start_pos:start_pos + n_samples][reward_mask[start_pos:start_pos + n_samples]] * (n_samples / (n_samples - 1)) - cur_reward_baseline
...
def compute_policy_loss(
    ...
    Args:
        ...
        off_policy_cliprange: (float)
            The clip range used in PPO for off-policy data.
    Returns:
    ...
    #     log_prob[~on_policy_mask] = torch.log(prob[~on_policy_mask] / (prob[~on_policy_mask] + 0.1))
    ...
    if on_policy_mask is not None:
        on_policy_mask = on_policy_mask.bool()
        ratio_safe = ratio.clone() # new tensor, same data
        if gamma_strategy == "constant":
            ratio_safe[~on_policy_mask] = ratio_safe[~on_policy_mask] / (
                ratio_safe[~on_policy_mask] + gamma_value
            )
        elif gamma_strategy == "percentile":
            off_policy_mask = ~on_policy_mask
            if off_policy_mask.any():                                
                gamma = torch.quantile(
                    ratio_safe[off_policy_mask].view(-1),
                    gamma_value
                ).clamp(min=1e-4).detach()
                ratio_safe[off_policy_mask] = ratio_safe[off_policy_mask] / (
                    ratio_safe[off_policy_mask] + gamma
                )
        elif gamma_strategy == "power":
            ratio_safe[~on_policy_mask] = torch.pow(ratio_safe[~on_policy_mask], gamma_value)
        else:
            raise ValueError(f"Unknown gamma strategy: {gamma_strategy}")
        ratio = ratio_safe
    ...
    # only apply clip range to on-policy data, off-policy data is not clipped
    clip_tensor = torch.ones_like(pg_losses)
    clip_tensor[~on_policy_mask] = 0
    pg_losses2 = -advantages * torch.where(on_policy_mask, torch.clamp(ratio, 1.0 - cliprange, 1.0 + cliprange), ratio)
    # pg_losses2 = -advantages * torch.where(~on_policy_mask, torch.clamp(ratio, 1.0 - off_policy_cliprange, 1.0 + off_policy_cliprange), pg_losses2)

    # for logging purposes only to check clipfrac for on and off policy data
    pg_losses_0_2 = -advantages * torch.clamp(ratio, 1.0 - 0.2, 1.0 + 0.2)
    ...
    pg_clipfrac = verl_F.masked_mean(torch.gt(pg_losses_0_2, pg_losses).float(), eos_mask)

    on_policy_mask = on_policy_mask.bool()
    
    if sum(on_policy_mask) > 0:
        on_pg_losses = verl_F.masked_mean(pg_losses[on_policy_mask], eos_mask[on_policy_mask])
        on_ppo_kl = verl_F.masked_mean(-negative_approx_kl[on_policy_mask], eos_mask[on_policy_mask])
        on_pg_losses_0_2 = pg_losses_0_2[on_policy_mask]
        on_pg_clipfrac = verl_F.masked_mean(torch.gt(on_pg_losses_0_2, on_pg_losses).float(), eos_mask[on_policy_mask])
    else:
        on_pg_losses = torch.tensor(0.0)
        on_ppo_kl = torch.tensor(0.0)
        on_pg_losses_0_2 = torch.tensor(0.0)
        on_pg_clipfrac = torch.tensor(0.0)
    
    if sum(~on_policy_mask) > 0:
        off_pg_losses = verl_F.masked_mean(pg_losses[~on_policy_mask], eos_mask[~on_policy_mask])
        off_ppo_kl = verl_F.masked_mean(-negative_approx_kl[~on_policy_mask], eos_mask[~on_policy_mask])
        off_pg_losses_0_2 = pg_losses_0_2[~on_policy_mask]
        off_pg_clipfrac = verl_F.masked_mean(torch.gt(off_pg_losses_0_2, off_pg_losses).float(), eos_mask[~on_policy_mask])
    else:
        off_pg_losses = torch.tensor(0.0)
        off_ppo_kl = torch.tensor(0.0)
        off_pg_losses_0_2 = torch.tensor(0.0)
        off_pg_clipfrac = torch.tensor(0.0)

    return pg_loss, pg_clipfrac, ppo_kl, on_pg_losses, off_pg_losses, on_ppo_kl, off_ppo_kl, on_pg_clipfrac, off_pg_clipfrac
