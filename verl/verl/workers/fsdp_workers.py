...
from verl.utils.aws_utils import distributed_aws_copy
...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        if not torch.distributed.is_initialized():
            torch.distributed.init_process_group(backend="nccl", timeout=timedelta(seconds=30 * 60))
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def upload_to_s3(self, local_path, s3_path, recursive=False, node_1_only=False):
        """
        Upload files or directories to S3 using Ray-based uploader to prevent OOM issues.
        Only the rank 0 process performs the actual upload.
        
        Args:
            local_path: Local file or directory path
            s3_path: S3 destination path (s3://bucket/path)
            recursive: Whether to upload directory recursively
        
        Returns:
            bool: True if upload was successful
        """

        print(f"Starting upload from {local_path} to {s3_path}")
        # Use ray actor-based upload to avoid memory issues
        
        if node_1_only and self.rank < 8:
            result = distributed_aws_copy(
                src=local_path, 
                dst=s3_path, 
                recursive=recursive, 
                use_s5cmd=False,  # Prefer boto3-based upload via Ray
                use_ray_actor=True,
            ...
            print(f"Upload to {s3_path} {'completed successfully' if result else 'failed'}")

        elif not node_1_only:
            result = distributed_aws_copy(
                src=local_path, 
                dst=s3_path, 
                recursive=recursive, 
        ...
        # Wait for rank 0 to finish upload
        torch.distributed.barrier()
        return True

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_checkpoint(self, local_path, global_step=0):
        ...
        self.checkpoint_manager.save_checkpoint(local_path=local_path, global_step=global_step)
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_huggingface_checkpoint(self, local_path, s3_path):
        # only support save and load ckpt for actor
        assert self._is_actor
        import torch
        if self._is_offload_param:
        ...
        self.checkpoint_manager.save_huggingface_checkpoint(local_path=local_path, s3_path=s3_path)
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def load_checkpoint(self, path, s3_path=None, del_local_after_load=True):
        if self._is_offload_param:
            load_fsdp_param_and_grad(module=self.actor_module_fsdp,
                                     device_id=torch.cuda.current_device(),
                                     load_grad=self._is_offload_grad)

        self.checkpoint_manager.load_checkpoint(path=path, s3_path=s3_path, del_local_after_load=del_local_after_load)
...
class PRIMERewardModelWorker(Worker):
    ...
    def __init__(self, config):
        ...
        world_size = torch.distributed.get_world_size()
        # TODO(sgm): support FSDP hybrid shard for larger model
        fsdp_size = self.config.prime_model.fsdp_config.get('fsdp_size', -1)
        self.device_mesh = create_device_mesh(world_size=world_size, fsdp_size=fsdp_size)
        ...
        # normalize config
        self.config.mini_batch_size *= self.config.prime_model.n_samples
        self.config.mini_batch_size //= (world_size // self.ulysses_sequence_parallel_size)
    ...
    def _build_model_optimizer(self, config, enable_gradient_checkpointing=False):
        ...
        # Check for remove padding support
        if self.use_remove_padding:
            from verl.models.registry import check_model_support_rmpad
            check_model_support_rmpad(model_config.model_type)
        ...
        torch_dtype = config.prime_model.fsdp_config.get('model_dtype', None)
        if torch_dtype is None:
        ...
        with init_context(), warnings.catch_warnings():
            ...
            # Apply Liger kernel if configured
            if config.prime_model.get('use_liger', False):
                from liger_kernel.transformers.monkey_patch import _apply_liger_kernel_to_instance
                _apply_liger_kernel_to_instance(model=reward_module)
        ...
        # Initialize the reference model if needed
        if config.prime_model.get("ref_type", 'freeze') == 'freeze':
            ref_path = copy_local_path_from_hdfs(config.prime_model.ref_path)
            with init_context(), warnings.catch_warnings():
                warnings.simplefilter("ignore")
                reference_module = AutoModelForCausalLM.from_pretrained(
                    pretrained_model_name_or_path=ref_path,
                    torch_dtype=torch.bfloat16,
                    attn_implementation='flash_attention_2',
                    trust_remote_code=trust_remote_code)
                ...
                if config.prime_model.get('use_liger', False):
                    from liger_kernel.transformers.monkey_patch import _apply_liger_kernel_to_instance
                    _apply_liger_kernel_to_instance(model=reference_module)
        ...
        mixed_precision_config = config.prime_model.fsdp_config.get('mixed_precision', None)
        if mixed_precision_config is not None:
            param_dtype = PrecisionType.to_dtype(mixed_precision_config.get('param_dtype', 'bf16'))
            reduce_dtype = PrecisionType.to_dtype(mixed_precision_config.get('reduce_dtype', 'fp32'))
            buffer_dtype = PrecisionType.to_dtype(mixed_precision_config.get('buffer_dtype', 'fp32'))
        ...
        # Get auto wrap policy for reward model
        auto_wrap_policy = get_fsdp_wrap_policy(module=reward_module, config=config.prime_model.fsdp_config.get('wrap_policy', None))
        
        fsdp_mesh = self.device_mesh
        sharding_strategy = get_sharding_strategy(fsdp_mesh)

        log_gpu_memory_usage('Before reward model FSDP init', logger=logger)
        
        # Wrap reward model with FSDP
        reward_module = FSDP(
            reward_module,
            param_init_fn=init_fn,
            use_orig_params=False,
        ...
        log_gpu_memory_usage('After reward model FSDP init', logger=logger)

        # Wrap reference model with FSDP if needed
        if reference_module is not None:
            ref_auto_wrap_policy = get_fsdp_wrap_policy(module=reference_module, 
                                                        config=config.prime_model.fsdp_config.get('wrap_policy', None))
            reference_module = FSDP(
                reference_module,
                param_init_fn=init_fn,
                use_orig_params=False,
                auto_wrap_policy=ref_auto_wrap_policy,
                device_id=torch.cuda.current_device(),
                sharding_strategy=sharding_strategy,
                mixed_precision=mixed_precision,
                sync_module_states=True,
        ...
        if self.update_dpo_type in ['before', 'after']:
            ...
            # Apply FSDP offload configurations
            if self._is_offload_optimizer:
                offload_fsdp_optimizer(optimizer=reward_optimizer)
        else:
            reward_optimizer = None
            reward_lr_scheduler = None

        # Apply param/grad offloading if configured
        if self._is_offload_param:
        ...
        log_gpu_memory_usage('After prime model build complete', logger=logger)
        return reward_module, reference_module, reward_optimizer, reward_lr_scheduler
    ...
    def init_model(self):
        ...
        self.reward_module, self.reference_module, self.reward_optimizer, self.reward_lr_scheduler = self._build_model_optimizer(
            config=self.config, 
        ...
        torch.cuda.empty_cache()
        log_gpu_memory_usage('After init_model complete', logger=logger)
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        with torch.no_grad(), torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                # For PRIME, handle the output appropriately
                # This needs to be adapted based on the specific structure of output from your model
                reward_rmpad = output.logits
                reward_rmpad = reward_rmpad.squeeze(0)  # (total_nnz)
                ...
                rm_score = output.logits  # (batch_size, seq_len, num_labels)
                rm_score = rm_score.squeeze(-1)
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        n_samples = data.meta_info.get('n_samples', 1)
        ...
        # Load parameters if using offloading
        ...
        # Use Ulysses sharding manager for better parallelism
        with self.ulysses_sharding_manager:
            rm_data = self.ulysses_sharding_manager.preprocess_data(data=rm_data)
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            
            # If we're not using PRIME's built-in update but want to do inference with dynamic batch sizing
            if self.update_dpo_type == 'none' and self.config.get('use_dynamic_bsz', False):
                use_dynamic_bsz = True
                max_token_len = self.config.get('forward_max_token_len_per_gpu', 1024) * self.ulysses_sequence_parallel_size
                micro_batches, indices = rearrange_micro_batches(batch=rm_data.batch, max_token_len=max_token_len)
                ...
                # Use PRIME's own implementation
                with Timer(name='prime_compute', logger=None) as timer:
                    token_level_scores, metrics = self.prm.update_policy(rm_data)
                    
                delta_time = timer.last
                if 'global_token_num' in data.meta_info:
                    global_num_tokens = data.meta_info['global_token_num']
                    if self.config.prime_model.get('flops_counter_enabled', False):
                        # Initialize FlopsCounter if needed and hasn't been done yet
                        if not hasattr(self, 'flops_counter'):
                            from transformers import AutoConfig
                            model_config = AutoConfig.from_pretrained(
                                copy_local_path_from_hdfs(self.config.prime_model.path),
                                trust_remote_code=self.config.prime_model.get('trust_remote_code', False)
                            )
                            self.flops_counter = FlopsCounter(model_config)
                            
                        estimated_flops, promised_flops = self.flops_counter.estimate_flops(global_num_tokens, delta_time)
                        metrics['mfu/prime'] = estimated_flops / promised_flops / self.world_size
                
                output = DataProto.from_dict(tensors={'rm_scores': token_level_scores}, meta_info={'metrics': metrics})
                
            output = self.ulysses_sharding_manager.postprocess_data(data=output)

        # Reshard and offload after computation
        if self.world_size > 1:
            self.reward_module._handle.reshard(True)
            if self.reference_module is not None:
                self.reference_module._handle.reshard(True)
        ...
        output = output.to('cpu')
        torch.cuda.empty_cache()
        log_gpu_memory_usage('After compute_rm_score', logger=logger)
        return output
    ...
    def upload_to_s3(self, local_path, s3_path, recursive=False, node_1_only=False):
        ...
        # Wait for rank 0 to finish upload
        # self.checkpoint_manager._file_based_barrier(local_path, message="upload_to_s3", s3_path=s3_path)
        torch.distributed.barrier()
        return True
    ...
    def save_checkpoint(self, local_path, global_step=0):
        ...
        # Use the checkpoint manager for consistent saving
        self.checkpoint_manager.save_checkpoint(local_path=local_path, global_step=global_step)
