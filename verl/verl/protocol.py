...
def fold_batch_dim(data: 'DataProto', new_batch_size):
    ...
    assert batch_size % new_batch_size == 0, f"batch_size {batch_size} must be divisible by new_batch_size {new_batch_size}"
...
class DataProtoItem:
    ...
    def slice(self, indices: List[int]) -> 'DataProtoItem':
        return DataProtoItem(batch=self.batch[indices], non_tensor_batch={key: val[indices] for key, val in self.non_tensor_batch.items()}, meta_info=self.meta_info)
...
class DataProto:
    ...
    @staticmethod
    def list_length(data: List['DataProto']) -> int:
        length = sum([len(d) for d in data])
        return length
    ...
    def check_consistency(self):
        ...
        """
        if self.batch is not None:
            assert len(self.batch.batch_size) == 1, f'only support num_batch_dims=1, but got {len(self.batch.batch_size)}'
        ...
        if self.batch is not None and len(self.non_tensor_batch) != 0:
            ...
            for key, val in self.non_tensor_batch.items():
                assert isinstance(
                    val, np.ndarray
                ) and val.dtype == object, f'data in the non_tensor_batch must be a numpy.array with dtype=object, but got {val.dtype}'
                assert val.shape[
    ...
    def from_dict(cls, tensors: Dict[str, torch.Tensor], non_tensors=None, meta_info=None, num_batch_dims=1):
        ...
        """
        # assert len(tensors) > 0, f'tensors must not be empty, but got {len(tensors)}'
        assert num_batch_dims > 0, f'num_batch_dims must be greater than zero, but got {num_batch_dims}'
        if non_tensors is not None:
            assert num_batch_dims == 1, f'only support num_batch_dims=1 when non_tensors is not None, but got {num_batch_dims}'
    ...
    def shuffle_n_samples(self, n_samples: int) -> 'DataProto':
        """Shuffle the batch with n_samples rows as one unit"""
        assert self.batch.batch_size[0] % n_samples == 0, f"{self.batch.batch_size[0]} % {n_samples} != 0"
        batch_size = self.batch.batch_size[0]
        shuffled_indices = torch.randperm(batch_size // n_samples)
        shuffled_batch = torch.cat([self.batch[i * n_samples:(i + 1) * n_samples] for i in shuffled_indices])
        shuffled_non_tensor_batch = {key: np.concatenate([self.non_tensor_batch[key][i * n_samples:(i + 1) * n_samples] for i in shuffled_indices]) for key in self.non_tensor_batch.keys()}
        shuffled_meta_info = {key: self.meta_info[key] for key in self.meta_info.keys()}
        return DataProto(batch=shuffled_batch, non_tensor_batch=shuffled_non_tensor_batch, meta_info=shuffled_meta_info)
