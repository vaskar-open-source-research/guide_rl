...
def _parse_latex(expr: str) -> str:
    ...
    # Handle escaped backslashes in input
    expr = expr.replace("\\\\", "\\")
    ...
    math_environments = [
        ...
        (r"\$\$(.+?)\$\$", r"\1"),         # $$...$$
        (r"\$(.+?)\$", r"\1"),             # $...$
        (r"\\begin\{equation\}(.+?)\\end\{equation\}", r"\1")  # \begin{equation}...\end{equation}
    ]
    
    for pattern, _ in math_environments:
    ...
    while "\\text{" in expr:
        text_match = re.search(r"\\text\{(.+?)\}", expr, re.DOTALL)
        if text_match:
            content = text_match.group(1)
            expr = expr.replace(text_match.group(0), content)
        else:
            break
    
    # Handle other common LaTeX commands similarly
    for cmd in ["textbf", "textit", "mathrm", "mathbf", "mathit"]:
        pattern = f"\\\\{cmd}\\{{(.+?)\\}}"
    ...
    try:
        ...
        print(f"Warning: LaTeX parsing failed: {e}")
        for char in "\\{}$_^":
...
def compute_score(model_output: str, ground_truth: str):
    ...
    return is_correct, is_matched, extracted_model_output
