...
# New implementation to replace pyext.RuntimeModule
class RuntimeModule:
    def __init__(self, name, code):
        self.__name__ = name
        self.__dict__ = {}
        try:
            # Execute the code string in a new dictionary as the module's namespace
            exec(code, self.__dict__)
        except Exception as e:
            raise SyntaxError(f"Failed to compile module {name}: {str(e)}") from e

    @classmethod
    def from_string(cls, name, filename, code):
...
class Capturing(list):
    def __enter__(self):
        ...
        # Don't override close method
        return self

    def __exit__(self, *args):
        self.append(self._stringio.getvalue())
        self._stringio.close()  # Properly close the StringIO
    ...
    if test is None:
        ...
        for index, inputs in enumerate(in_outs["inputs"]):
            ...
            if which_type == CODE_TYPE.call_based:  # Call-based
                ...
                if in_outs["outputs"][index] == "$assert%":
                    tmp_result = True
                    results.append(tmp_result)
                    continue
