...
def _temp_run(sample, generation, debug, result, metadata_list):
    ...
    with open(os.devnull, "w") as devnull:
        ...
        sys.stderr = devnull
        try:
            res, metadata = run_test(in_outs=sample, test=generation, debug=debug)
            result.append(res)
    ...
    # set stdout and stderr to original
    # sys.stdout = original_stdout
    # sys.stderr = original_stderr
...
def check_correctness(in_outs: Optional[dict], generation, timeout=10, debug=True):
    ...
    p = multiprocessing.Process(
        target=_temp_run, args=(in_outs, generation, debug, result, metadata_list)
    )
    p.start()
    ...
    # result = []
    # metadata_list = []
    # _temp_run(in_outs, generation, debug, result, metadata_list)
    # return result[0], metadata_list
