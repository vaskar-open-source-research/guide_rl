...
def compute_score_single(completion, test_cases):
...
def compute_score(completion, test_cases, strict=False):
    ...
    try:
        ...
        for i in range(len(inputs)):
            entry = {"inputs": [inputs[i]], "outputs": [outputs[i]]}
            if "fn_name" in test_cases:
                entry["fn_name"] = test_cases["fn_name"]
            test_cases_list.append(entry)
        ...
        for test_case_id, test_case in enumerate(test_cases_list):
            res, metadata = apps_check_correctness(
                in_outs=test_case, generation=solution, timeout=1, debug=False
            )
            try:
                metadata = dict(enumerate(metadata))[0] if metadata else {}  # metadata can be empty occasionally
            except Exception as e:
                metadata = {}
            metadata["test_case"] = {}
