...
EXPERIMENT_NAME='qwen-2.5-math-7b-guide'
DATA_PATH='verl/experiments/math_reasoning/data/guidance_data_full'
LOCAL_SFT_MODEL_PATH='Qwen/Qwen2.5-Math-7B'
CKPT_PATH='./ckpt'
S3_PATH='s3://scale-ml/genai/maple/guide-open-source-test'
...
uv run --active --directory $base_dir python -m verl.trainer.main_ppo --config-name=ppo_trainer\
    ...
    scale_reasoning.response_format=qwen_format \
    ...
    scale_reasoning.reroll_with_hints=True \
    scale_reasoning.reroll_with_cot=False \
    actor_rollout_ref.actor.importance_weighted_reroll=True \
    scale_reasoning.do_verify_format=True \
    scale_reasoning.reroll_threshold=0.001 \
    actor_rollout_ref.actor.clip_ratio=0.2 \
    actor_rollout_ref.actor.off_policy_clip_ratio=0.8 \
    actor_rollout_ref.rollout.max_num_batched_tokens=4096 \
    actor_rollout_ref.actor.apply_off_policy_shaping=True \
    actor_rollout_ref.actor.gamma_strategy=percentile \
    actor_rollout_ref.actor.gamma_value=0.9 \
    scale_reasoning.custom_chat_template=qwen_math_chat_template \
